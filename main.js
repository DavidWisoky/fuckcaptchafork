function disableCaptcha() {
    document.querySelector("#postform > div.postform__raw.postform__raw_flex.captcha").style.pointerEvents = "none"
    document.querySelector("#qr-postform > div.postform__raw.postform__raw_flex.captcha").style.pointerEvents = "none"

    let postform = document.querySelector("#postform > div.postform__raw.postform__raw_flex.captcha > input.captcha__val.input")
    let qrPostform = document.querySelector("#qr-postform > div.postform__raw.postform__raw_flex.captcha > input.captcha__val.input")

    postform.readOnly = qrPostform.readOnly = true
    postform.style.fontSize = qrPostform.style.fontSize = "12px"
    postform["placeholder"] = qrPostform["placeholder"] = "fuck 2ch captcha (plugin)"
    postform.style.pointerEvents = qrPostform.style.pointerEvents = "none"
}

function doLoopTillWorking() {
    try {
        let url = document.querySelector("#captcha-widget-main > img")['src'];
        clearInterval(pollingFunc);
        if (url != lastUrl || lastCaptchaFailed) {
            scrapCaptchaImage(url);
        }
        lastUrl = url
    }
    catch (err) {
...
function scrapCaptchaImage(url) {
    toDataURL(url)
        .then(dataUrl => { sneed(dataUrl) })
}

function sneed(a) {
    fetch('https://hf.space/embed/davidwisoky/dvatch_captcha_sneedium_fork2/api/predict', {
        method: 'POST',
        headers: {
            'Accept': '*/*',
...
function handleResponse(response, do_polling){
    ...
    var clear_me_please_interval = setInterval(function() {
      ...
      if (a.includes(post_success)){
        ...
        // }, 100);
        setTimeout(function() {
          pollingFunc = setInterval(function() {doLoopTillWorking();}, 900);
        }, 900);
      ...
      // if (a.includes(captcha_cancelled)  ){
      //     setTimeout(function() {
      //       // $("#qr-submit").click()
      //       $("#submit").click()
      //     }, 100);
...
let memeColors = setInterval(function() {
  try{
    // node1.style.backgroundColor  = randomHexColor();
    // node2.style.backgroundColor  = randomHexColor();
